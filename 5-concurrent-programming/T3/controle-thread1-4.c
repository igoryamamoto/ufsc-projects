...
char port1[]="1234";
double buffer_T[BUFSIZE];
double buffer2_T[BUFSIZE];
double buffer_N[BUFSIZE];
double buffer2_N[BUFSIZE];
int bufferFull = 0;
int bufferUnload;

pthread_t cntN, cntT, tela, saveN, saveT, saveTela, ref,insBufferT;
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t mutexCtr = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t wait_buffer = PTHREAD_COND_INITIALIZER;
...
void finsBufferT(){
    ...
    if (n_buffer == 0){
        buffer1_T[pos] = f_sensor("st-0");
        pos++;
    }
    else{
        buffer2_T[pos] = f_sensor("st-0");
        pos++;
    }

    if (pos == BUFSIZE){
        bufferUnload = n_buffer;
        n_buffer = !n_buffer;
        pos = 0;
        bufferFull = 1;
        pthread_cond_signal(&wait_buffer);
    }
...
void fsaveT(){
    ...
    while(1){
        ...
        while(bufferFull == 0){
            pthread_cond_wait(&wait_buffer, &mutexCtr);
        }
...
int main()
{
	...
	pthread_create(&insBufferN,NULL,(void *)finsBufferT,NULL);

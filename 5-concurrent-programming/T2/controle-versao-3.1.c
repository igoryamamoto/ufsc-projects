...
void armazena_tempos(){
	int i;
	FILE *pf;
	if((pf = fopen("TemposResposta.txt","a")) == NULL){ /* Abre arquivo binário para escrita */
		printf("Erro na abertura do arquivo");
		exit(1);	
	}
	fprintf(pf, "Tempos de Resposta - Controlador Nível \n");
	for(i=0;i<NMAX;i++) {
		fprintf(pf, "%f\n",tempo_N[i]);
	}
...
int main()
{
	...
	scanf ("%f",&Tref); 
		...
		if(aux%7==0 & in<NMAX){
			...
			t_resposta = tn.tv_sec - t.tv_sec;
			t_resposta += (tn.tv_nsec - t.tv_nsec)*1000;
		...
		if(aux%9==0 & it<NMAX){
			...
			t_resposta = tt.tv_sec - t.tv_sec;
			t_resposta += (tt.tv_nsec - t.tv_nsec)*1000;
